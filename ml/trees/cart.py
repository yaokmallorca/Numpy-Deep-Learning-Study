...
class Leaf(object):
    """
    leaf is an array of class probability if classifer is True
    else the mean of the region
    """
    def __init__(self, value):
        self.value = value

class DecisionTree(object):
    def __init__(self, classifier=True, max_depth=None, num_classes=None,
                num_features=None, criterion='entropy', seed=None):
        if seed:
            np.random.seed(seed)
        self.root = None
        self.curr_depth = 0
        ...
        if classifier and criterion == "mse":
            raise ValueError("`mse` is a valid criterion only for regression.")
    ...
    def _grow(self, X, y):
        ...
        N, M = X.shape
        self.curr_depth += 1
        feat_idx = np.random.choice(M, self.n_feats,replace=False)

        # greedily select the best split according to cirterion
        feat, thresh = self._segment(X, y, feat_idx)
    ...
    def _segment(self, X, y, feat_idx):
        """
        Find the optimal split rule (feature index and split threshold)
        for the data according to criterion
        """
        best_gain = -np.inf
...
def test_DecisionTree_regression():
    ...
    # while True:
    # for i in range(0, 10):
    n_ex = 20 # np.random.randint(2, 100)
    n_feats = 2 # np.random.randint(2, 100)
    max_depth = 10 # np.random.randint(1, 5)
    ...
    mine = DecisionTree1(
        criterion=criterion, max_depth=max_depth, classifier=False
    )
    ...
    print("mine: ", y_pred_mine)
    y_pred_gold = gold.predict(X)
    print("sklearn: ", y_pred_gold)


test_DecisionTree_regression()