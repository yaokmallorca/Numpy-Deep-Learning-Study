...
def calc_pad_dims_2D(X_shape, out_dim, kernel_shape, stride, dilation=0):
	"""
	Compute the padding necessary to ensure that convolving 'X' with a 
	2D kernel of shape 'kernel_shape' and stride 'stride' produces outputs
	with dimension 'out_dim'

	params:
	X_shape: tuple of (n_ex, in_rows, in_cols, in_ch)
		dimensions of the input volumn. padding is applied to 'in_rows' 
		and 'in_cols'
	out_dim: tuple of (out_dim, out_cols)
		the disired dimension of an output example after applying the 
		convolution
	...
	return: 4-tuple
		padding dims for 'X'. organised as (left, right, up, down)
	"""
...
def calc_pad_dims_1D(X_shape, l_out, kernel_width, stride, dilation=0, causal=False):
	...
	of shape 'kernel_shape' and stride 'stride' produces outputs with length 'l_out'
	
	params:
	X_shape: tuple (n_ex, l_in, in_ch)
		dimensions of the input volumn. Padding is applied on either side of 'l_in'.
	l_out: int
		the disired length an output example after applying the convolution
	kernel_width: int
		the with of the 1D convolution kernel
	stride: int
		the stride of the convolution kernel
	dilation: int
		number of pixels inserted between kernel elements. defautl is 0
	causal: bool
		whether to compute the padding dims for a regular or causal convolution.
		if causal, padding is added only to the left side of the sequence. defautl 
		is False.

	return:
	padding_dims: 2-tuple
		padding dims for X organised as (left, right)
	"""
	if not isinstance(X_shape, tuple):
...
def pad1D(X, pad, kernel_width=None, stride=None, dilation=0):
	"""
	Zero-pad a 3D input volumn 'X' along the second dimension
	Params:
	X: shape is (n_ex, n_in, in_ch), 
		input volumn, padding is applied to 'l_in'
	pad: tuple, int, or {'same', 'causal'}
		the padding amount. if 'same', add padding to ensure that the output length 
		of 1D convolution with a kernel of 'kernel_shape' and stride 'stride' is the 
		as the input length. if 'causal' compute padding such that the output both has 
		the same length as the input and 'ouput[t' does not depend on 'input[t+1'. if 
		2-tuple, specifies the number of padding columns to add on each side of the 
		sequence.
	kernel_width: int
		the dimension of the 2d convolution kernel. Only relevant if p=same or causal.
		default is none
	sride: int
	dilation: int

	return:
	x_pad: (n_ex, padded_seq, in_channels)
	p: 2d-tuple
		the number of 0-padded columns added to the (left, right) of the sequence in 
	'X'.
	"""
	X_pad = None
	p = pad
...
def pad2D(X, pad, kernel_shape=None, stride=None, dilation=0):
	"""
	Zero-pad a 4D input volumn 'X' along the second and third dimensions.
	
	Params:
	X: (n_ex, n_rows, n_cols, in_ch)
		input volumn. padding is applied to 'in_rows' and 'in_cols'.
	...
	Return:
	X_pad: (n_ex, padded_in_rows, padded_in_cols, in_channels)
		the padded output volumn.
	p: 4-tuple
		the number of 0-padded rows added to the (top, bottom, left, right) 
		of 'X'
	"""
	X_pad = None
	p = pad
...
def dilate(X, d):
	...
	Return:
	xd: (n_ex, out_rows, out_cols, out_ch)
		the dilated array where
		out_{rows} = in_{rows} + d(in_{rows}-1)
		out_{cols} = in_{cols} + d(in_{cols}-1)
	"""
...
def calc_fan(weight_shape):
	"""
	compute the fan-in and fan-out for a weight matrix
...
def calc_conv_out_dims(X_shape, W_shape, stride=1, pad=0, dilation=0):
	"""
	compute the dimension of the output volumn for the specified convolution

	params:
	X_shape: 3-tuple or 4-tuple
		the dimensions of the input volumn to the convolution. 
		3-tuple: (n_ex, in_length, in_ch)
		4-tuple: (n_ex, in_rows, in_cols, in_ch)
	W_shape: 3-tuple or 4-tuple
		3-tuple: (fr_len, in_ch, out_ch)
		4-tuple: (fr, fc, in_ch, out_ch)
	pad: tuple, int, or {'same', 'causal'}
	...
	out_dims: 3-tuple or 4-tuple
		the dimension of output volumn
	"""
...
def _im2col_indices(X_shape, fr, fc, p, s, d=0):
	"""
	helper function that computes indices into X in prep for columnization 
	in 'im2col'
...
def im2col(X, W_shape, pad, stride, dilation=0):
	...
	Notes
	A numpy reimaginning of matlabs im2col sliding function
	Code extended from Andrej Karpathy's ``im2col.py``.
	...
	W_shape: 4-tuple (kernel_rows, kernel_cols, in_ch, out_ch)
		the dimension of the weights/kernels in the present convolutional 
		layer
	pad: tuple, int
		the padding amount
	stride: int
	...
	Return:
	X_col: (Q, Z)
		the reshaped input volumn where
...
def col2im(X_col, X_shape, W_shape, pad, stride, dilation=0):
	"""
	take columns of a 2D matrix and rearrage them into the block of a 4D image volumn
	...
	X_shape: 4-tuple (n_ex, in_rows, in_cols, in_ch)
		the original dimension of X (not including padding)
	W_shape: (kernel_rows, kernel_cols, in_ch, out_ch)
		the dimensions of the weight in the present convolutional layer
	pad: 4-tuple (left, right, up, down)
	stride: int
	dilation: int

	return:
	img: (n_ex, in_rows, in_cols, in_ch)
...
def conv1D(X, W, stride, pad, dilation=0):
	...
	p2D = (0, 0, p[0], p[1])
	Z2D = conv2D(X2D, W2D, sride, p2D, dilation)
